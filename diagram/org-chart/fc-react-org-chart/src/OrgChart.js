...
export default class OrgChart {
  constructor(options) {
    ...
    if (opts.exportButton && !chartContainer.querySelector('.oc-export-btn')) {
      ...
      exportBtn.setAttribute(
        'class',
        'oc-export-btn' + (opts.chartClass !== '' ? ' ' + opts.chartClass : '')
      );
      exportBtn.innerHTML = 'Export';
      exportBtn.addEventListener('click', this._clickExportButton.bind(this));
      downloadBtn.setAttribute(
        'class',
        'oc-download-btn' + (opts.chartClass !== '' ? ' ' + opts.chartClass : '')
      );
  ...
  getRelatedNodes(node, relation) {
    if (relation === 'parent') {
      ...
      return Array.from(this._closest(node, el => el.nodeName === 'TABLE').lastChild.children).map(
        el => el.querySelector('.node')
      );
    } else if (relation === 'siblings') {
      return this._siblings(this._closest(node, el => el.nodeName === 'TABLE').parentNode).map(el =>
        el.querySelector('.node')
      );
    }
    return [];
  }
  ...
  _startLoading(arrow, node) {
    ...
    let exportBtn = this.chartContainer.querySelector(
      '.oc-export-btn' + (opts.chartClass !== '' ? '.' + opts.chartClass : '')
    );
  ...
  hideChildren(node) {
    ...
    let descendants = Array.from(lastItem.querySelectorAll('.node')).filter(el =>
        that._isVisible(el)
      ),
  ...
  _buildSiblingNode(nodeChart, nodeData, callback) {
    ...
    if (nodeChart.parentNode.nodeName === 'TD') {
      ...
      that._buildChildNode.call(
        that,
        that._closest(nodeChart.parentNode, el => el.nodeName === 'TABLE'),
        nodeData,
        () => {
          if (++childCount === newSiblingCount) {
            let siblingTds = Array.from(
              that._closest(nodeChart.parentNode, el => el.nodeName === 'TABLE').lastChild.children
            );
            ...
            if (existingSibligCount > 1) {
              ...
              that._addClass(siblingTds, 'hidden');
              that._addClass(
                that._getDescElements(siblingTds.slice(0, insertPostion + 1), '.node'),
                'slide-right'
              );
              that._addClass(
                that._getDescElements(siblingTds.slice(insertPostion + 1), '.node'),
                'slide-left'
              );
            }
            callback();
      ...
      that.buildHierarchy.call(that, that.chart, nodeData, 0, () => {
        if (++nodeCount === siblingCount) {
          let temp = nodeChart.nextElementSibling.children[3].children[insertPostion],
  ...
  _clickHorizontalEdge(event) {
    ...
    if (siblingsState.exist) {
      ...
      if (opts.toggleSiblingsResp) {
        let prevSib = this._closest(node, el => el.nodeName === 'TABLE').parentNode
            .previousElementSibling,
          nextSib = this._closest(node, el => el.nodeName === 'TABLE').parentNode
            .nextElementSibling;
    ...
    if (this.chart.style.transform) {
      ...
      if (isFirefox) {
        ...
        ghostNodeWrapper.src =
          'data:image/svg+xml;utf8,' + new XMLSerializer().serializeToString(ghostNode);
  ...
  _onDrop(event) {
    ...
    if (dragColSpan > 2) {
      ...
      let dragSibs = Array.from(dragZone.parentNode.parentNode.children[3].children).map(function(
        td
      ) {
  ...
  _onPanStart(event) {
    ...
    if (
      this._closest(event.target, el => el.classList && el.classList.contains('node')) ||
